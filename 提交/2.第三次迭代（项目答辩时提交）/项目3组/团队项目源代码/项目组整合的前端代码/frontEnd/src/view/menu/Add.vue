  ...
  <div>
    <el-card >
      ...
      <el-form :model="menuDetail" :rules="rules" ref="ruleFormRef">
        ...
        <el-form-item label="菜单英文名:" prop="ename">
...
const rules = reactive({
  ...
  ename: [
    { required: true, message: "菜单英文名不能为空", trigger: "blur" },
    {
      validator: function (rule, value, callback) {
        if (/^[a-zA-Z]+$/.test(value) == false) {
          callback(new Error('菜单英文名必须为英文字母'));
        } else {
          callback();
        }
      },
  ...
  pid: [{ required: true, message: "菜单父id不能为空", trigger: "blur" }],
