        ...
        <el-card class="box-card">
            ...
            <el-form :model="menuDetail" :rules="rules" ref="ruleFormRef">
                <el-form-item label="菜单ID:">
                  <el-input v-model="menuDetail.menuId" style="width:300px" :readonly="true"  class="input-with-select"></el-input>
                </el-form-item>
              ...
              <el-form-item label="菜单请求地址:">
                <el-input v-model="menuDetail.path" style="width:300px" placeholder="请输入菜单请求地址" class="input-with-select"></el-input>
              ...
              <el-form-item label="菜单父ID:">
                <el-input readonly v-model="menuDetail.pid" style="width:300px;enable:false" placeholder="请单击选择父菜单" class="input-with-select" ></el-input>
                <el-button type="primary" @click="showDialog(menuDetail.pid)">选择</el-button>
              </el-form-item>

              <el-form-item label="菜单排序:">
                <el-input v-model="menuDetail.sort" style="width:300px" placeholder="请输入菜单英文名" class="input-with-select"></el-input>

              </el-form-item>
              <p v-if="route.query.menuId=='' || route.query.edit=='1'">
...
onBeforeMount(async () => {
    if (route.query.menuId) {
        const res = await userApi.getMenuDetail({ menuId: route.query.menuId })
        console.log(res.data.data);
      menuInfo.value= res.data.data;
      menuDetail.menuId = res.data.data[0].menuId;
      menuDetail.cname = res.data.data[0].cname;
      menuDetail.ename = res.data.data[0].ename;
      menuDetail.path = res.data.data[0].path;
      menuDetail.component = res.data.data[0].component;
      menuDetail.icon = res.data.data[0].icon;
      menuDetail.pid = res.data.data[0].pid;
      menuDetail.sort = res.data.data[0].sort;
    }
})
...
const onSubmit = () => {
  ...
  ruleFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      const res = await userApi.updateMenu(menuDetail);

      if (res.data!=null) {
        if (res.data) {

          ElMessage.info("菜单修改成功");
...
const showDialog = async(id) => {
  ...
  if(res.data!=null){
    ...
    defaultCheckedKeys.value[0]=menuDetail.pid;
  }
